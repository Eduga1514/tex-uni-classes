...
\begin{teo}
    Sean $A \subseteq \R^n$ abierto, conexo y convexo, con $f: A \rightarrow \R$ de clase $C^2(A)$\marginfootnote{Es decir, que $f$, $\partial_if, \partial^2_{ij}f$ para todo $i,j=1, \dots, n$ existen en cada punto de $A$ y además son continuas.}. Entonces
...
En el caso particular en que $u = e_i$, con $i = 1, \dots, n$, tenemos que $\partial_u f(x_0)$ es la derivada parcial.
